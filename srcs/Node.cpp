...
void	Node::setMap(const unsigned short * const * map) {
...
Node::Node(size_t size) : size(size) {
	this->map = new unsigned short*[size];
	this->map[0] = new unsigned short[size * size];
	for (size_t i = 1; i < size; i++)
		this->map[i] = this->map[0] + size * i;
}

Node::Node(const unsigned short * const * map, int size)
	: Node(size)
{
	this->setMap(map);
	this->hash();
...
Node::Node(const unsigned short * const * map, int size, int cost, int heuristic, unsigned short pos0[2], Node *parent)
	: size(size),
	  cost(cost),
	  heuristic(heuristic),
	  parent(parent),
	  pos0 {pos0[0], pos0[1]}
{
	...
	this->map = new unsigned short*[this->size];
	for (size_t i = 0; i < this->size; i++)
	{
		this->map[i] = new unsigned short[this->size];
	}
	this->setMap(map);
	this->hash();
}
...
Node::Square	Node::find0(const unsigned short * const * map, size_t size) {
	Square pos0;
	...
	for (size_t y = 0; y < this->size; y++)
	{
		for (size_t x = 0; x < this->size; x++)
		{
			if ((int)x == this->pos0.x && (int)y == this->pos0.y)
				std::cout << "\033[38;5;124m" << (int)this->map[y][x] << "\033\1330m ";
			else
				std::cout << "\033[38;5;34m" << (int)this->map[y][x] << "\033\1330m ";
		}
		std::cout << std::endl;
	}
	std::cout << std::endl;
...
unsigned short*	Node::square(const Square& s) const {
