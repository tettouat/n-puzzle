...
Solver::Solver(unsigned short **map, unsigned short **finalMap, int size, IHeuristic& heuristic)
	: _size(size), _pool(size), _openlist(&Solver::less_node),
	  _closelist(100, &Solver::hash_node, &Solver::eq_node),
	  _firstNode(map, size),
...
const Node&		Solver::lastNode(void) { return (*this->_lastNode); }

size_t	Solver::hash_node(const Node* node) {
	return (node->_hash);
}

bool	Solver::eq_node(const Node* a, const Node* b) {
	return (*a == *b);
}

bool	Solver::less_node(const Node* a, const Node* b) {
	if (a->distance == b->distance)
		return (a->cost < b->cost);
	return (a->distance > b->distance);
}

size_t	Solver::getTotalStates () {
	return (this->_totalStates);
}

size_t	Solver::getMaxStates() {
	return (this->_maxStates);
}

constNodes	Solver::buildPath(Node* node) const
{
	constNodes	lst;
...
constNodes	Solver::buildPath(void) const
{
	return (this->buildPath(this->_lastNode));
}

constNodes	Solver::buildMultiPath(const Solver& fromStart, const Solver& fromEnd) {
	constNodes	ret;
...
void	Solver::createNextNodes(int size, Node* topNode, NodeTool& pool) {
...
unsigned short	**Solver::getSnakeForm(unsigned short **map, int size) {
...
bool	Solver::isSolvable(unsigned short **map, int size) {
	unsigned short	**newMap = getSnakeForm(map, size);
	int		total_size = size * size;
	int		count = 0;
...
unsigned short	**Solver::finalSolution(int size) {
	unsigned short	**newMap = new unsigned short*[size];
	int		total_size = size * size;
	unsigned short	**snakeMap;
	...
	snakeMap = getSnakeForm(newMap, size);
	for (int i = 0; i < total_size - 1; i++)
	{
		*(snakeMap[i]) = i + 1;
	}
...
unsigned short	**Solver::genMap(size_t size, size_t swaps)
{
	int signedSize = size;
...
bool	Solver::collide(const Solver& a, const Solver& b)
{
	auto	found = b._closelist.find(a._lastNode);

	if (found != b._closelist.end() )
	{
		return (true);
	}
	return (false);
